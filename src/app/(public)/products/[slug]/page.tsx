      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          <div className="w-full lg:w-1/2 space-y-4">
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl overflow-hidden aspect-[4/3] relative shadow-2xl border border-white/50 dark:border-slate-800/50 p-3 group">
              <div 
                className="absolute inset-3 rounded-2xl bg-slate-100 dark:bg-slate-800 bg-cover bg-center shadow-inner transition-transform duration-700 group-hover:scale-[1.02]" 
            ...
            {product.images && product.images.length > 1 && (
              <div className="grid grid-cols-4 gap-4">
                {product.images.map((img: string, idx: number) => (
                  <div key={idx} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl overflow-hidden aspect-square relative border border-white/50 dark:border-slate-800/50 hover:border-brand-500 hover:shadow-lg transition-all cursor-pointer p-1.5 group">
                    <div 
                      className="absolute inset-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 bg-cover bg-center transition-transform duration-500 group-hover:scale-110" 
          ...
          {/* Product Info */}
          <div className="w-full lg:w-1/2 flex flex-col bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-3xl p-8 border border-white/40 dark:border-slate-800/50 shadow-xl">
            <div className="mb-2">
              <Link href={`/categories/${product.category?.slug || ''}`}>
                <span className="inline-block px-4 py-1.5 bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 rounded-full text-xs font-bold uppercase tracking-widest mb-4 border border-brand-200 dark:border-brand-800/50 hover:bg-brand-200 dark:hover:bg-brand-900/60 transition-colors cursor-pointer shadow-sm">
                  {product.category?.name || 'Uncategorized'}
                </span>
              </Link>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold text-slate-900 dark:text-white mb-6 leading-tight">
                {product.name}
              </h1>
              ...
              <div className="flex items-center gap-6 text-slate-600 dark:text-slate-400 mb-8 border-b border-slate-200 dark:border-slate-800 pb-8">
                {product.origin && (
                  <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800/50 px-4 py-2 rounded-full shadow-inner">
            ...
            <div className="prose prose-slate dark:prose-invert max-w-none mb-10">
              <h3 className="text-2xl font-bold font-serif mb-4 text-slate-900 dark:text-white">Product Overview</h3>
            ...
            <div className="mb-10">
              <h3 className="text-2xl font-bold font-serif mb-6 text-slate-900 dark:text-white flex items-center gap-3">
                <Scale className="h-6 w-6 text-brand-500" /> Technical Specifications
              </h3>
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-md">
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                    {product.specs && Object.keys(product.specs).length > 0 ? Object.entries(product.specs).map(([key, value]) => (
                      <tr key={key} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        <td className="py-4 px-6 text-sm font-bold text-slate-800 dark:text-slate-200 w-1/3 bg-slate-50/50 dark:bg-slate-900/50 border-r border-slate-200 dark:border-slate-800">{key}</td>
                        <td className="py-4 px-6 text-sm text-slate-600 dark:text-slate-400 font-medium">{value as string}</td>
                      </tr>
                    )) : (
                      <tr>
            ...
            {/* Actions */}
            <div className="mt-auto bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-950 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-lg relative overflow-hidden group">
              ...
              <h4 className="font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2 text-xl">
                <Package className="h-6 w-6 text-brand-500" /> Ready to Source?
              </h4>
              <p className="text-slate-600 dark:text-slate-400 mb-8">Get in touch to discuss bulk pricing, compliance documents, and custom logistics for your destination.</p>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Link
                  href="/contact"
                  className="flex-1 inline-flex items-center justify-center px-6 py-4 text-base font-bold rounded-2xl text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-sm hover:shadow-md hover:-translate-y-1 gap-2"
                >
                  Send Formal Inquiry
                </Link>
              </div>
