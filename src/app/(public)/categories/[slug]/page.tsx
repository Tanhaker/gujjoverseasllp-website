...
export default async function CategoryDetailPage({ params }: { params: Promise<{ slug: string }> }) {
 ...
 <div 
 className="relative pt-32 pb-24 lg:pt-40 lg:pb-32 overflow-hidden border-b border-slate-200 "
 ...
 {/* Breadcrumbs */}
 <nav className="flex text-sm text-slate-500 font-medium mb-10 bg-white/50 backdrop-blur-md inline-flex px-4 py-2 rounded-full shadow-sm border border-slate-200/50 ">
 ...
 <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 shrink-0 transform hover:scale-105 transition-transform duration-300">
 {renderIcon(category.icon, category.color || '#2ecc71')}
 </div>
 <div>
 <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-slate-900 mb-4 tracking-tight">
 {category.name}
 </h1>
 <p className="text-lg md:text-xl text-slate-700 max-w-3xl leading-relaxed font-light">
 ...
 <div>
 <h2 className="text-3xl font-serif font-bold text-slate-900 mb-2">Available Products</h2>
 <p className="text-slate-500 font-medium">Showing {displayProducts.length} items</p>
 </div>
 <Link href="/products" className="hidden sm:flex text-brand-600 hover:text-brand-700 font-bold items-center gap-2 bg-brand-50 px-5 py-2.5 rounded-full transition-colors hover:bg-brand-100 ">
 View Full Catalog <ArrowRight className="w-4 h-4" />
 </Link>
 </div>

 {displayProducts.length === 0 ? (
 <div className="bg-white/50 backdrop-blur-sm rounded-3xl border border-slate-200/50 p-16 text-center shadow-sm">
 ...
 <Link key={product.id} href={`/products/${product.slug}`} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100/80 group relative flex flex-col hover:-translate-y-2">
 ...
 <div className="h-64 bg-slate-100 relative overflow-hidden shrink-0">
 <div 
 ...
 <h3 className="text-xl font-bold text-slate-900 mb-3 group-hover:text-brand-600 transition-colors line-clamp-2">{product.name}</h3>
 <p className="text-slate-500 line-clamp-2 mb-6 text-sm flex-grow leading-relaxed">
 {product.short_description}
 </p>
 <div className="flex items-center text-brand-600 font-semibold mt-auto group-hover:text-brand-700 text-sm">
 View Details 
