...
export default async function AdminLayout({
 ...
 <div className="p-6 border-b border-slate-800">
            <Link href={isSuperAdmin ? "/secure/superadmin/dashboard" : "/secure/admin/dashboard"} className="flex items-center gap-3 mb-4 group">
              <div className="relative h-10 w-10 overflow-hidden flex items-center justify-center bg-white/5 rounded-full p-1.5 border border-white/10 group-hover:scale-105 transition-transform">
                <img src="/logo.png" alt="Logo" className="h-full w-full object-contain filter drop-shadow-md brightness-110" />
              </div>
              <div className="flex flex-col">
                <span className="font-serif font-bold text-white text-lg leading-tight">
                  {isSuperAdmin ? 'SuperAdmin' : 'Admin Portal'}
                </span>
                <span className="text-[10px] tracking-widest text-brand-500 uppercase font-medium">Gujj Overseas</span>
              </div>
            </Link>
