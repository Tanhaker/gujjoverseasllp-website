...
export default function CategoriesManager() {
  ...
  const EditorRow = () => (
    <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-brand-200 dark:border-brand-900 mb-6 shadow-sm">
      <h3 className="text-lg font-bold mb-4 text-slate-900 dark:text-white">{isAdding ? 'Add New Category' : 'Edit Category'}</h3>
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-6">
          ...
          <div className="flex gap-2 flex-wrap">
            {ICONS.map(i => (
              <button 
                ...
                className={`p-2 rounded-lg border ${formData.icon === i.name ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/30 text-brand-600' : 'border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
              >
                {i.component}
              </button>
            ))}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Color Accent</label>
          <div className="flex gap-2 flex-wrap">
            {COLORS.map(c => (
              <button 
                key={c}
                onClick={() => setFormData({...formData, color: c})}
                className={`w-8 h-8 rounded-full shadow-sm ring-2 ring-offset-2 dark:ring-offset-slate-900 ${formData.color === c ? 'ring-slate-400' : 'ring-transparent'}`}
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      <div className="p-6">
        {(isAdding || editingId) && <EditorRow />}

        <div className="space-y-3">
          {categories.map((cat) => (
            <div 
              key={cat.id} 
              className={`flex items-center gap-4 p-4 rounded-xl border transition-colors ${!cat.is_visible ? 'border-dashed border-slate-300 dark:border-slate-700 opacity-60' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-brand-300 dark:hover:border-brand-700'}`}
            >
              ...
              <div className="flex-grow">
                <div className="flex items-center gap-2">
                  ...
                  {!cat.is_visible && <span className="text-[10px] uppercase bg-slate-200 dark:bg-slate-800 px-2 py-0.5 rounded font-bold text-slate-500">Hidden</span>}
                </div>
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="px-3 py-1.5 text-sm font-medium border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400"
                >
                  {cat.is_visible ? 'Hide' : 'Show'}
                </button>
                <button 
                  onClick={() => startEdit(cat)}
                  className="p-2 text-slate-500 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/30 rounded-lg transition-colors"
                >
