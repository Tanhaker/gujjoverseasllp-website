...
export default function InquiriesList() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      {/* Filters Bar */}
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-wrap gap-4 items-center justify-between bg-slate-50 dark:bg-slate-900">
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 border border-slate-300 dark:border-slate-700 rounded-lg text-sm bg-white dark:bg-slate-950 text-slate-900 dark:text-white"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <select 
              ...
              className="px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg text-sm bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-300"
            >
              ...
              inquiries.map(inq => (
                <tr key={inq.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-900 dark:text-white">{inq.customer_name}</div>
                  ...
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-900 dark:text-slate-300">{inq.product_name}</div>
                  ...
                  <td className="px-6 py-4">
                    <select 
                      ...
                      className="text-xs font-medium px-2 py-1 border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-900 cursor-pointer"
                    >
                      <option value="new">New</option>
                      <option value="contacted">Contacted</option>
                      <option value="negotiating">Negotiating</option>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <a 
                        ...
                        className="p-1.5 text-slate-400 hover:text-green-500 bg-slate-100 dark:bg-slate-800 rounded-lg transition-colors"
                      ...
                      <a 
                        ...
                        className="p-1.5 text-slate-400 hover:text-blue-500 bg-slate-100 dark:bg-slate-800 rounded-lg transition-colors"
                        title="Reply via Email"
                      >
                        <Mail className="w-4 h-4" />
                      </a>
                      <select 
                        className="text-xs bg-slate-100 dark:bg-slate-800 border-none rounded-lg p-1.5 text-slate-600 cursor-pointer w-24"
                        value={inq.status}
